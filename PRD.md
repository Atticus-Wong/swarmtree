...
- Create a Node CLI with subcommands:
  ...
  - [x] `swarmtree status <task-id>`: show git status, branch, commits ahead/behind, and task metadata.
  - [x] `swarmtree done <task-id>`: mark complete after optional validation notes.
  - [x] `swarmtree clean <task-id>`: remove completed worktree only after explicit confirmation.
